    ...
    private function growthTrends(): array
    {
        return DB::select("
            SELECT DATE_TRUNC('week', o.order_date)::date AS week,
                ...
                LAG(SUM(o.total)) OVER (ORDER BY week) AS prev_revenue,
                CASE WHEN LAG(SUM(o.total)) OVER (ORDER BY week) > 0 THEN ROUND(((SUM(o.total)-LAG(SUM(o.total)) OVER (ORDER BY week))/
                  LAG(SUM(o.total)) OVER (ORDER BY week)*100)::numeric, 1) ELSE 0 END AS growth_pct
            FROM orders o
