...
export default function CreateDebitNotePage() {
  ...
  const { today, allowPast } = useVoucherTxnDate('accounts.debit_notes.backdate');
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push('/dashboard/accounts/debit-notes');
    } catch (x: unknown) {
  ...
  return (
    <PermissionGuard permission="accounts.debit_notes.create">
      <div className="space-y-6">
        <PageHeader title="Add Debit Note" description="Increases debtor outstanding." />
