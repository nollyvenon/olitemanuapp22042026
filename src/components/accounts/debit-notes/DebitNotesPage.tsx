...
type Row = { id: string; debit_note_number?: string; amount: string | number; date: string };
...
export default function DebitNotesPage() {
  ...
  useEffect(() => {
    void api.get('/debit-notes').then((r) => {
  ...
  return (
    <PermissionGuard permission="accounts.debit_notes.read">
          ...
          <tbody>
            {rows.map((x) => (
              <tr key={x.id} className="border-b">
                <td className="p-2 font-mono">{x.debit_note_number}</td>
