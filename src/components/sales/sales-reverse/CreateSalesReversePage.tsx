...
type Inv = { id: string; invoice_number: string; status: string; customer_id?: string; total?: number };

export default function CreateSalesReversePage() {
  const router = useRouter();
  const api = getApiClient();
  const { today, allowPast } = useVoucherTxnDate('sales.sales_reverses.backdate');
  const [custs, setCusts] = useState<Opt[]>([]);
  const [leds, setLeds] = useState<Opt[]>([]);
  const [invs, setInvs] = useState<Inv[]>([]);
  const [form, setForm] = useState({ invoice_id: '', customer_id: '', ledger_account_id: '', amount: '', reason: '', date: today });
    ...
    void (async () => {
      ...
      setInvs(
        inv
          ...
          .map((i: Inv & { customer_id?: string; total?: number }) => ({
            id: i.id,
  ...
  useEffect(() => {
    ...
    const inv = invs.find((i) => i.id === form.invoice_id);
    if (inv?.customer_id) {
      setForm((f) => ({ ...f, customer_id: inv.customer_id!, amount: inv.total != null ? String(inv.total) : f.amount }));
    }
  }, [form.invoice_id, invs]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push('/dashboard/sales/sales-reverse');
    } catch (x: unknown) {
  ...
  return (
    <PermissionGuard permission="sales.sales_reverses.create">
      <div className="space-y-6">
        <PageHeader title="Create Sales Reverse" description="Cancel invoiced sale; marks invoice reversed." />
        {err && <p className="text-sm text-red-600">{err}</p>}
        <form onSubmit={submit} className="max-w-xl space-y-3 rounded border p-4">
          <div>
            <Label>Invoice *</Label>
            <select required className="w-full rounded border px-2 py-2 text-sm" value={form.invoice_id} onChange={(e) => setForm({ ...form, invoice_id: e.target.value })}>
              <option value="">Select</option>
              {invs.map((i) => <option key={i.id} value={i.id}>{i.invoice_number}</option>)}
            </select>
          </div>
          <div>
