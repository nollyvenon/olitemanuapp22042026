...
const SETTINGS: Setting[] = [
  ...
  { key: 'base_currency',   label: 'Base Currency',        value: 'NGN',                     type: 'select', options: ['NGN', 'USD', 'EUR', 'GBP'] },
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (settings) {
          const settingsObj = Array.isArray(settings) ? Object.fromEntries(settings.map((s: any) => [s.key, s.value])) : settings;
          setValues(prev => ({ ...prev, ...settingsObj }));
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (err.response?.status === 404) {
        alert('⚠️ Settings endpoint not yet available on backend.\n\nChanges are stored locally. Backend integration coming soon.');
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-lg border-l-4" style={{ background: '#fff8e7', borderColor: '#FF9900' }}>
        <p className="text-sm font-medium" style={{ color: '#c45500' }}>
          ℹ️ Settings API Integration: Backend endpoint not yet configured. Changes are stored locally in the UI.
        </p>
      </div>
      ...
      <div className="bg-white rounded-lg overflow-hidden" style={{ border: '1px solid #d5d9d9' }}>
        ...
        <div className="px-5 py-4 flex justify-end" style={{ background: '#f4f6f8', borderTop: '1px solid #d5d9d9' }}>
          <Button onClick={handleSave} disabled={saving} style={{ background: '#FF9900', color: '#0f1111' }} className="font-semibold hover:opacity-90">
